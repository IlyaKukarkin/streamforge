...
class StreamForgeServer {
	...
	private setupWebSocketServer(): void {
		...
		this.wsServer.on("connection", (ws: WebSocket, request: any) => {
			...
			ws.on("close", () => {
				...
				logger.info(
					`WebSocket client disconnected. Active connections: ${this.connectedClients.size}`,
				);
			});
	...
	public async start(): Promise<void> {
		try {
			...
			const wsPort =
				this.config.websocket?.port || this.config.websocketPort || 3001;
	...
	private processDonationEffect(donation: any): void {
		try {
			...
			switch (donation.eventType) {
				...
				default:
					logger.warn("Unknown donation event type:", {
						eventType: donation.eventType,
					});
			}
			...
			logger.error("Error processing donation effect:", {
				error: (error as Error).message,
			});
		}
	}
