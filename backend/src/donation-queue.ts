...
import type { DonationEvent, DonationEventType, Config } from './types/index.js';
...
export class DonationQueue {
   ...
   */
  getCooldownStatus(): Record<DonationEventType, { 
    active: boolean; 
    remainingMs: number;
    lastEventTime: number;
  }> {
    ...
    return status as Record<DonationEventType, { 
      active: boolean; 
      remainingMs: number;
      lastEventTime: number;
    }>;
   ...
   */
  async processQueue(handler: (donation: DonationEvent) => Promise<void>): Promise<void> {
    if (this.processing) {
    ...
    try {
      while (!this.isEmpty()) {
        ...
        try {
          ...
          console.error(`Failed to process donation ${donation.donationId}:`, error);
  ...
  private initializeCooldowns(): void {
    const eventTypes: DonationEventType[] = ['BOOST', 'SPAWN_ENEMY', 'HEAL', 'SPAWN_DRAGON'];
...
export function getDonationQueue(): DonationQueue {
  if (!queueInstance) {
    throw new Error('DonationQueue not initialized. Call createDonationQueue() first.');
  }
  return queueInstance;
}