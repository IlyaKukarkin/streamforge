...
export function loadConfig(): Config {
  const config: Config = {
    ...
    donationRateWindow: parseInt(process.env.DONATION_RATE_WINDOW ?? '60000', 10),
    donationPerUserLimit: parseInt(process.env.DONATION_PER_USER_LIMIT ?? '3', 10),
    boostCooldown: parseInt(process.env.BOOST_COOLDOWN ?? '1', 10),
    spawnEnemyCooldown: parseInt(process.env.SPAWN_ENEMY_COOLDOWN ?? '5', 10),
    spawnDragonCooldown: parseInt(process.env.SPAWN_DRAGON_COOLDOWN ?? '30', 10),
    enableMetrics: process.env.ENABLE_METRICS?.toLowerCase() === 'true',
    enableFileLogging: process.env.ENABLE_FILE_LOGGING?.toLowerCase() === 'true',
...
function validateConfig(config: Config): void {
  ...
  if (config.websocketPort < 1 || config.websocketPort > 65535) {
    errors.push(`Invalid WEBSOCKET_PORT: ${config.websocketPort} (must be 1-65535)`);
  }
  ...
  if (config.donationRateLimit < 1 || config.donationRateLimit > 1000) {
    errors.push(`Invalid DONATION_RATE_LIMIT: ${config.donationRateLimit} (must be 1-1000)`);
  }

  if (config.donationRateWindow < 1000 || config.donationRateWindow > 3600000) {
    errors.push(`Invalid DONATION_RATE_WINDOW: ${config.donationRateWindow} (must be 1s-1h)`);
  }

  if (config.donationPerUserLimit < 1 || config.donationPerUserLimit > 100) {
    errors.push(`Invalid DONATION_PER_USER_LIMIT: ${config.donationPerUserLimit} (must be 1-100)`);
  }

  if (config.boostCooldown < 0 || config.boostCooldown > 300) {
    errors.push(`Invalid BOOST_COOLDOWN: ${config.boostCooldown} (must be 0-300s)`);
  }

  if (config.spawnEnemyCooldown < 1 || config.spawnEnemyCooldown > 300) {
    errors.push(`Invalid SPAWN_ENEMY_COOLDOWN: ${config.spawnEnemyCooldown} (must be 1-300s)`);
  }

  if (config.spawnDragonCooldown < 1 || config.spawnDragonCooldown > 3600) {
    errors.push(`Invalid SPAWN_DRAGON_COOLDOWN: ${config.spawnDragonCooldown} (must be 1-3600s)`);
  }
