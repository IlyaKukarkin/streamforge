...
import type { LogLevel, LogEntry, Config } from '../types/index.js';
...
export class Logger {
   ...
   */
  logDonation(donationId: string, eventType: string, viewerId: string, amount: number, status: string): void {
    this.info('Donation event processed', {
      donationId,
      eventType,
      viewerId,
      amount,
   ...
   */
  logGameState(gameId: string, status: string, score: number, health: number): void {
    this.debug('Game state updated', {
      gameId,
      status,
      score,
      health,
