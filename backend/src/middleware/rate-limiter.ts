...
import type { Config, RateLimitState, RateLimitEntry } from '../types/index.js';
...
export class RateLimiter {
  ...
  getStatus(): {
    ...
    return {
      global: {
        ...this.globalState,
        remaining: Math.max(0, this.config.donationRateLimit - this.globalState.count),
      },
  ...
  getUserStatus(userId: string): RateLimitEntry & { remaining: number } {
    ...
    return {
      ...userState,
      remaining: Math.max(0, this.config.donationPerUserLimit - userState.count),
    };
  }
