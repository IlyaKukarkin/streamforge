...
import type { GameState, GameStatus, DonationEvent, Knight, Enemy } from './types/index.js';
...
export class GameStateManager {
  ...
  applyBoost(durationSeconds: number, boostPercent: number = 50): GameState {
    ...
    const finalExpiry = this.gameState.boostActive && currentExpiry > Date.now()
  ...
  checkBoostExpiry(): GameState {
    if (this.gameState.boostActive && Date.now() >= this.gameState.boostExpiry) {
  ...
  updateFromGame(updates: {
    ...
    if (updates.knightHealth !== undefined) {
      validatedUpdates.knightHealth = Math.max(0, Math.min(100, updates.knightHealth));
    }
  ...
  getStats(): {
    ...
    const uptime = this.gameState.startTime > 0 
...
export function getGameStateManager(): GameStateManager {
  if (!gameStateInstance) {
    throw new Error('GameStateManager not initialized. Call createGameStateManager() first.');
  }
  return gameStateInstance;
}